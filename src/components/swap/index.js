...
class Swap extends React.Component {
  ...
  constructor(props) {
    ...
    this.fetch_quote = this.fetch_quote.bind(this);
  ...
  on_change_pay_value(e) {
    ...
    const pay_value_timeout = setTimeout(() => {
      ...
      // TODO Warning!: fetch quote with takerAddress
      ...
      axios
        ...
        .catch((err) => {
          // TODO: err.response.data.code = 100 when insufficient liq
  ...
  async on_click_button(e) {
    ...
    if (this.state.button_state === 'CONNECT') {
      ...
      const url_query_price_pay = `?buyToken=${
        pay_token.address
      }&sellToken=${usdt_address}&sellAmount=${sell_amount}&chain_id=${Number(
        chain_id
      )}`;
    ...
    if (this.state.button_state === 'SWAP') {
      ...
      */
      ...
      while (true) {
        res_quote = await this.fetch_quote(url_quote);
      ...
      const extra_gas_buffer = 0.35; // TODO: edit gas buffer

      res_quote.data.gas = parseInt(
        Number(res_quote.data.estimatedGas) +
          Number(res_quote.data.estimatedGas) * extra_gas_buffer
      ).toString();
...
Swap.defaultProps = {
  ...
  parentWidth: false, // For resizing component according to its parent width
  baseURL: config.url_api + '/v1/swap', // we keep the base url open ended incase developers wants to implement 0x.org API on their back end, for implementing your own 0x.org API check the docs: https://docs.devchain.net
};
